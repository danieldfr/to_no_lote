...
class SearchExample {
  ...
  var pointers;
  String address = '';
  SearchExample(HereMapController hereMapController, lat, long, _mapMarkerList,
      address, _streamController, pointers)
      : _hereMapController = hereMapController,
        _mapMarkerList = _mapMarkerList,
        address = address,
        lat = lat,
        long = long,
        _streamController = _streamController,
        pointers = pointers,
        _camera = hereMapController.camera {
    double distanceToEarthInMeters = 20000;
    ...
    _pickMapMarker(GeoCoordinates(lat!, long!));
    print("recebendo $pointers");
    print("tamanho: ${pointers.length}");
    _pickMapItem();
  }
  ...
  void _searchExample() {
    String searchTerm = "avenida brasil, Palmas";
  ...
  void _setTapGestureHandler() {
    _hereMapController.gestures.tapListener = TapListener((Point2D touchPoint) {
      ...
      _pickMapMarker(_hereMapController.viewToGeoCoordinates(touchPoint));
    });
  }
  ...
  Future<void> _getAddressForCoordinates(GeoCoordinates geoCoordinates) async {
    ...
    _searchEngine.searchByCoordinates(geoCoordinates, reverseGeocodingOptions,
      ...
      atualizarTexto(address);
    });
  }

  Future<void> _pickMapMarker(geoCoordinates) async {
    // _clearMap();
    _addPoiMapMarker(geoCoordinates);
    // _getAddressForCoordinates(geoCoordinates);
  }

  Future<void> _pickMapItem() async {
    _addPoiMapMarker(GeoCoordinates(pointers[0]['lat'], pointers[0]['long']));
    for (int i = 0; i < pointers.length; i++) {
      print("-------------------");
      print("cordenadas ${pointers[i]['lat']}, ${pointers[i]['long']}");
      // await _pickMapMarker(
      //     GeoCoordinates(pointers[i]['lat'], pointers[i]['long']));
      print("Final");
    }
  }
  ...
  Future<void> _searchInViewport(String queryString) async {
    ...
    searchOptions.languageCode = LanguageCode.ptBr;
    ...
    _searchEngine.searchByText(query, searchOptions,
      ...
      for (Place searchResult in list) {
        ...
        // await _getAddressForCoordinates(searchResult.geoCoordinates!);
        double distanceToEarthInMeters = 1000 * 2;
        MapMeasure mapMeasureZoom =
            MapMeasure(MapMeasureKind.distance, distanceToEarthInMeters);
        _camera.lookAtPointWithMeasure(
            searchResult.geoCoordinates!, mapMeasureZoom);
      }
    });
  }
  ...
  Future<Uint8List> _loadFileAsUint8List(String fileName) async {
    ...
    ByteData fileData = await rootBundle.load('imagens/' + fileName);
  ...
  GeoBox _getMapViewGeoBox() {
    GeoBox? geoBox = GeoBox(
      GeoCoordinates(-33.75, -74.25),
      GeoCoordinates(5.25, -34.8),
    );
  ...
  void atualizarTexto(address) {
    _streamController.add(address);
  }

  String get addressMap => address;

  String get atLat => lat;
  String get atLong => long;
}
